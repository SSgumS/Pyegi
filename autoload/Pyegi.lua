...
script_authors = "DrunkSimurgh & SSgumS"
...
function main(sub,sel)
	...
	local str_res = tostring(res.tscript)
    if btn=="Apply" then
		local psya = sc_dir..str_res.."/".."gui_params.json" --the path for the yaml file containing controls' spec
		...
		if btn2 == btns[1] then
			local save_table = controls_table
			for _, items1 in pairs(save_table.Controls) do
				items1.value = res2[items1.name]
			end
			local save_str = json.encode(save_table)
			--Writing the created string to a file.
			local out = assert(io.open(sc_dir..str_res.."/".."gui_returned_values.json", "wb"))
			out:write(save_str)
			assert(out:close())
			...
			local pspa = sc_dir..str_res.."/"..scandir_f(sc_dir..str_res, "py") --the path for the python script
			--local pspa = "C:/Users/saman/AppData/Roaming/Aegisub/automation/autoload/PythonScripts/SimpleGradient/script.py"
			local handle = assert(io.popen(python_path.." "..pspa.." "..sc_dir.." "..str_res))
			handle:close()
			l365 = sub[10]
			l365.text = str_res
			sub[0] = l365
