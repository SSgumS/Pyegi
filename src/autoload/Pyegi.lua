...
local function post_init(sub, sel)
	...
	local _, status, _, _ = petzku_utils.io.run_cmd('"' ..
		dependency_dir .. 'Pyegi/.venv/' .. python_relative_path .. '" -s -u ' .. command_parameters_string)
	assert(status)
	...
	if file_exists(main_py_parameters_file_path) then --TODO: address the scenarios in which this could happen
		...
		-- write the project properties to a file
		local aegisub_project_properties_file_path = os.tmpname()
		local aegisub_project_properties = assert(io.open(aegisub_project_properties_file_path, "wb"))
		aegisub_project_properties:write(tostring(serialize(aegisub.project_properties())))
		assert(aegisub_project_properties:close())

		-- get the project details and convert it to string
		local project_details_str = tostring(serialize(aegisub.project_properties()))
		-- write the project details to a temp file
		local project_details_file_path = os.tmpname()
		-- aegisub.log(5, serialize(project_details_file_path) .. "\n")
		local project_details = assert(io.open(project_details_file_path, "wb"))
		project_details:write(project_details_str)
		assert(project_details:close())
		...
		local command_parameters_string = ' "' .. py_script_path ..
			...
			'" "' .. aegisub_project_properties_file_path .. '"'
		-- aegisub.log(5, serialize(command_parameters_string) .. "\n")
