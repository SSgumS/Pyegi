...
class Ui_MainWindow(object):
    ...
    def openWindow(self, script_name):
        ...
        if self.AllLines_radioButton.isChecked():
            selected_lines_and_flag['isSelected'] = "False"
        else:
            selected_lines_and_flag['isSelected'] = "True"
        json.dump(selected_lines_and_flag, open(system_inputs[4], "w"))
        if widgets['Format'].lower() == "lua":
            ...
            os.system(
                f'""{dependency_dir}.venv/Scripts/python.exe" "{scriptPath}{script_name}/main.py" "{system_inputs[1]}" "{system_inputs[2]}" "{system_inputs[4]}""')
